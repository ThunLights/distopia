...
export const load = (async (e) => {
	...
	for (const element of peoples) {
		elementsScore[element.userId] = DangerousPeople.strArrToScore(
			await database.dangerousPeople.score.fetch(element.userId)
		);
		elementsTag[element.userId] = (
			await database.dangerousPeople.tag.findUserTags(element.userId)
		).map((value) => value.content);
		elementsSubAccounts[element.userId] = (
			await database.dangerousPeople.subAccount.fetch(element.userId)
		).map((value) => value.userId);
	}

	return {
		count: await database.dangerousPeople.countAll(),
		peoples: peoples.map((value) => {
			return {
				...value,
				...{
