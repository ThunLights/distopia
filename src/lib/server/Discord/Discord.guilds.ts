...
export class GuildClient {
	...
	public async guildMemberAdd(member: GuildMember): Promise<void> {
		try {
			...
			if (dangerousPeople) {
				const notice = await database.guildTables.settings.dangerousPeople.notice.fetch(
					member.guild.id
				);
				...
				const channel =
					registeredChannels && registeredChannels.type === ChannelType.GuildText
						? registeredChannels
				...
				const embed = new EmbedBuilder()
					...
					.setTitle("危険人物の加入を検知")
					.setDescription(
						`「${member.user.username}(ID: ${member.user.id})」は危険人物に登録されています。`
					)
					.addFields(
						{
							name: "詳細情報",
							value: `通称: ${dangerousPeople.name}, 理由: ${dangerousPeople.title}`,
							inline: false
						},
						{
							name: "危険度スコア",
							value: `${score}(${isBanUser ? "BAN対象" : "BAN対象外"})`,
							inline: false
						},
			...
			if (
				guild &&
				!(
					this.lateLimits.user.includes(member.id) ||
					this.lateLimits.guild.includes(member.guild.id)
				)
			) {
				if (!this.lateLimits.user.includes(member.id)) this.lateLimits.user.push(member.id);
				if (!this.lateLimits.guild.includes(member.guild.id))
					this.lateLimits.guild.push(member.guild.id);
				...
				setTimeout(() => {
					...
					this.lateLimits.guild = this.lateLimits.guild.filter(
						(value) => value !== member.guild.id
					);
				}, 16 * 1000);
			}
		} catch {
