...
export class BumpRoleCancelButton extends ButtonsBase {
	...
	public async commands(interaction: ButtonInteraction<CacheType>): Promise<void | string | MessagePayload | InteractionReplyOptions | ButtonsError | null> {
		try {
			...
			const result = await database.guildTables.settings.bumpNoticeRole.delete(interaction.guild.id);
			return { content: result ? "Bump通知時にロールをメンションしなくなりました" : "DATABASE_ERROR", flags: [ MessageFlags.Ephemeral ] } satisfies InteractionReplyOptions;
		} catch (error) {
			errorHandling(error);
