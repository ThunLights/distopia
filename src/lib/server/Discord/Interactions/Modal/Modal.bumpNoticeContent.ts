...
export class BumpNoticeContent extends ModalsBase {
	...
	public async commands(interaction: ModalSubmitInteraction<CacheType>): Promise<(InteractionReplyOptions & { fetchReply: true; }) | string | MessagePayload | ModalsError | null> {
		try {
			...
			if (blank(content)) {
				return { content: "空白のみは無効です。", flags: [ MessageFlags.Ephemeral ] } satisfies InteractionReplyOptions;
			}
			const result = await database.guildTables.settings.bumpNoticeContent.update(interaction.guildId, content.trim());
			if (!result) {
				return { content: "DATABASE_ERROR", flags: [ MessageFlags.Ephemeral ] } satisfies InteractionReplyOptions;
			}
			return { content: `以下に設定しました。\n${codeBlock(content.trim())}`, flags: [ MessageFlags.Ephemeral ] } satisfies InteractionReplyOptions;
		} catch (error) {
			errorHandling(error);
