...
export class SettingsCommand extends CommandsBase {
	...
	public async commands(
		...
		if (
			...
			const autoBan = await database.guildTables.settings.dangerousPeople.ban.fetch(
				interaction.guild.id
			);
			const noticeChannel = await database.guildTables.settings.dangerousPeople.notice.fetch(
				interaction.guild.id
			);
			const bumpRole = await database.guildTables.settings.bumpNoticeRole.fetch(
				interaction.guild.id
			);
			const actingOwner = await database.guildTables.settings.owner.fetch(interaction.guild.id);
			const bumpNoticeContent = await database.guildTables.settings.bumpNoticeContent.fetch(
				interaction.guild.id
			);

			const bumptNoticeValue = bumpNotice ? (bumpNotice.content ? "有効" : "無効") : "未設定";
			const autoBanValue = autoBan ? `危険度スコア${autoBan.score}以上でBAN` : "未設定";
			const noticeChannelValue = noticeChannel
				? `出力チャンネル: <#${noticeChannel.channelId}>`
				: "未設定";
			const bumpRoleValue = bumpRole ? `<@&${bumpRole.roleId}>` : "未設定";
			const actingOwnerValue = actingOwner ? `<@${actingOwner.userId}>` : "未設定";
			const bumpNoticeContentValue = bumpNoticeContent ? bumpNoticeContent.content : "未設定";
