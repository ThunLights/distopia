	...
	async commands(
		...
		if (interaction.user.id === PUBLIC_OWNER_ID) {
			...
			if (commandName === "ticket") {
				...
				return {
					embeds: [embed],
					components: [new ActionRowBuilder<ButtonBuilder>().addComponents(ticketCreateButton)]
			...
			if (commandName === "score") {
				...
				const result = await database.panel.dangerousPeopleScore.update(
