	...
	async commands(
		...
		if (commandName === "register") {
			if (
				...
				if (!(interaction.channel && interaction.channel.type === ChannelType.GuildText)) {
					return {
						content: "テキストチャンネル以外に招待リンクを設定することはできません",
				...
				if (guild instanceof DatabaseError) {
					return {
						content: "登録チェック時にデータベースエラーが発生しました。",
						flags: [MessageFlags.Ephemeral]
					} satisfies InteractionReplyOptions;
				}
				if (guild) {
				...
				if (!result) {
					return {
						content: "データベースへの書き込みがうまく行きませんでした",
						flags: [MessageFlags.Ephemeral]
					} satisfies InteractionReplyOptions;
				}
				const embed = new EmbedBuilder()
					...
					.setDescription(
						`現在招待リンクは、このチャンネルに設定されています。正式登録後に変えることが出来ます。`
					)
					.setFields({
						name: "本登録について",
						value: "本登録は https://distopia.top/ にて行う事が出来ます。"
					});
		...
		if (commandName === "invite") {
			if (
				...
				if (this.changeInviteRateLimit.includes(interaction.guildId)) {
					return {
						content: "頻繁にサーバー招待URLを変えることはできません",
						flags: [MessageFlags.Ephemeral]
					} satisfies InteractionReplyOptions;
				}
				if (
				...
				setTimeout(
					() => {
						this.changeInviteRateLimit = this.changeInviteRateLimit.filter(
							(value) => value !== interaction.guildId
						);
					},
					10 * 60 * 1000
				);
		...
		if (commandName === "page") {
			...
			if (guild instanceof DatabaseError || guild === null) {
				return {
					content: "サーバーが見つかりませんでした。正式登録が済んでいるか確認してみてください。",
					flags: [MessageFlags.Ephemeral]
				} satisfies InteractionReplyOptions;
			}
			if (guild) {
				return {
					content: `https://distopia.top/guilds/${guild.guildId}`
				} satisfies InteractionReplyOptions;
			}
		}
